...
class YorkshireEncryption:
    ...
    def decrypt(cls, key: bytes, msg: bytes) -> bytes:
        ...
        cipher = AES.new(key, AES.MODE_CBC, iv = iv)
        ptxt = cls.unpad(cipher.decrypt(ctxt))
...
class BackupServer(CommandServer):
    ...
    def place_flag(self):
        ...
        ```
        """
        self.secret_file = f"{secrets.randbelow(10000)}: don't forget that this is your secret AC login code.".encode() + b" " * 32
    ...
    def handle_list(self, msg):
        ...
        try:
            self.send_message({"result": list(ctxt_hash.hex() for ctxt_hash in self.db.keys())})
        except (KeyError, ValueError, TypeError):
            self.send_message({"error": f"Invalid parameters."})
    ...
    def handle_check(self, msg):
        ...
        try:
            ...
            self.send_message({"result": ctxt_start == self.db[ctxt_hash][:16]})
        except (KeyError, ValueError, TypeError):
            self.send_message({"error": f"Invalid parameters."})
    ...
    def handle_flag(self, msg):
        try:
            ...
            self.send_message({"result": f"Clearly you just got lucky! Do it again {self.totalsolves - self.numsolves} times!"})
            self.db = defaultdict(bytes)
            self.place_flag()
